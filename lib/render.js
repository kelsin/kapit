...
exports.help = function() {
  ...
  output += '\n {green-fg}s{/} edit script (in editor)';
  output += '\n {green-fg}u{/} edit url';
...
exports.script = function(request) {
  var script = '';
  if(request.script) {
    try {
      return cardinal.highlight(JSON.stringify(JSON.parse(request.script || '{}'),
                                               null,
                                               2),
                                { json: true });
    }
...
exports.http = function(request, label) {
  ...
  if (request) {
    ...
    if(label) {
      output += '{yellow-fg}' + label + '{/}\n';
    }

    output += '{green-fg}' + request.method + '{/} ' + request.url + '\n';
...
exports.request = function(step) {
  ...
  if (step.type === 'HTTP' && !data.raw()) {
    ...
    if (step.request.script) {
      output += '\n\n';
      output += '{blue-fg}Script{/}\n';
      output += exports.script(step.request);
    }
    ...
    try {
      outputStep.request.script = JSON.parse(step.request.script);
    }
    catch (e) {
      outputStep.request.script = _.cloneDeep(step.request.script);
    }
